platform :ios, '10.0'

use_modular_headers!
inhibit_all_warnings!
workspace 'TokenD-Plus'
source 'https://github.com/CocoaPods/Specs.git'
source 'git@github.com:tokend/ios-specs.git'

def main_app_pods
  pod 'TokenDSDK', '3.1.0-rc.10'
  pod 'TokenDSDK/AlamofireNetwork'
  pod 'TokenDSDK/JSONAPI'
  pod 'TokenDSDK/AlamofireNetworkJSONAPI'
  pod 'TokenDSDK/KeyServer'
  
  pod 'QRCodeReader.swift',       '8.1.1'
#  pod 'ReachabilitySwift',        '~> 4.1'
  pod 'RxCocoa',                  '~> 4.1'
  pod 'RxSwift',                  '~> 4.1'
  pod 'SnapKit',                  '~> 4.0'
  pod 'SwiftKeychainWrapper',     '3.0.1'
  pod 'ActionsList', 		  '~> 0.9.1'
  
  pod 'PullToRefresher', '~> 3.0'
  pod 'Nuke'
  pod 'MarkdownView', 	 '1.5.0'
  pod 'AFDateHelper', '~> 4.2.2'
  
  pod 'Firebase/Core'
  pod 'Fabric'
  pod 'Crashlytics'
  
  pod 'Charts', git: 'https://github.com/tokend/Charts.git'
  pod 'Floaty', git: 'https://github.com/tokend/Floaty.git'
  pod 'UICircularProgressRing'
end

target 'TokenD-Plus' do
  main_app_pods

  post_install do |installer|
    # targetsToDisableBitcode = %w[TokenDSDK DLCryptoKit]
    #
    # installer.pods_project.targets.each do |target|
    #   next unless targetsToDisableBitcode.include? target.name
    #
    #   target.build_configurations.each do |config|
    #     config.build_settings['ENABLE_BITCODE'] = 'NO'
    #   end
    # end

    swift4Targets = ['QRCodeReader.swift', 'ActionsList']
    
    installer.pods_project.targets.each do |target|
      next unless swift4Targets.include? target.name
      
      target.build_configurations.each do |config|
        config.build_settings['SWIFT_VERSION'] = '4'
      end
    end

    # copy Acknowledgements
    require 'fileutils'
    FileUtils.cp_r('Pods/Target Support Files/Pods-TokenD-Plus/Pods-TokenD-Plus-acknowledgements.markdown', 'TokenD-Plus/Resources/acknowledgements.markdown')
  end
end
